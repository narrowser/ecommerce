...
export function authMiddleware(req: Request, _res: Response, next: NextFunction) {
  ...
  if (!header || !header.startsWith('Bearer ')) return next(new UnauthorizedError('Missing or invalid token'));
  try { req.user = jwt.verify(header.split(' ')[1], config.jwtSecret) as JwtPayload; next(); }
